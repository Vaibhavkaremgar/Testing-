...
def enhanced_fallback_evaluation(resume_text: str, job_title: str, job_description: str, 
    ...
    # Get job skill map for required skills
    skill_map = JOB_SKILL_MAPS.get(job_title.lower().strip(), JOB_SKILL_MAPS["default"])
    required_skills = skill_map.get("core", [])[:10]  # Top 10 required skills
...
def simulate_resume_parsing(candidate: Candidate, db: Session, ai_analysis: dict = None):
    ...
    if ai_analysis:
        ...
        if 'key_strengths' in ai_analysis and ai_analysis['key_strengths']:
            ...
            candidate.skills = skills[:20] if skills else candidate.skills
        ...
        if score >= 70:
            ...
            candidate.stage = CandidateStage.POTENTIAL_FIT
        elif score >= 35:
